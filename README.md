...
当前版本定位为“简洁版电商短视频编导 Agent”，前台专注真人实拍脚本和 AI 视频脚本生成，不再把素材学习、视频解析、参考仿写等实验能力混入主工作台。

前台已从“多工作区”回归为单一的“脚本生成工作台”。“爆款素材学习 / 视频素材解析”功能已从前台隐藏，相关代码和测试暂时保留，后续如有需要可独立拆成单独工具。
...
当前前台页面已回归为“简洁版脚本生成工作台”。用户打开网页后只看到产品信息输入、示例产品快速载入、脚本模式、脚本子类型、生成模式、输出结果、一键复制、Markdown / JSON 导出、历史记录和 metadata。
